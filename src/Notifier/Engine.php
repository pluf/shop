...
class Notifier_Engine implements JsonSerializable
{
    ...
    public function getType()
    {
        ...
        // NOTE: hadi, 2019: all notifier backend sould be placed in the determined folder
        if (strpos($name, Notifier_Engine::ENGINE_PREFIX) !== 0) {
            throw new Notifier_Exception_EngineLoad('NotifierEngine class must be placed in engine package.');
        }
        return substr($name, strlen(Notifier_Engine::ENGINE_PREFIX));
    }
     ...
     * 
     * @param array $data
     * @return boolean true if sending notification is successful else false.
     */
     ...
     *
     * Each notifier engine needs some parameters which should be provided by the creator.
